...
export default function RandomNumber({ isVerifiableMode, walletAddress }: RandomNumberProps) {
        ...
        <div className="relative px-6 py-4 bg-black rounded-xl flex items-center justify-center gap-3 font-press-start text-sm text-white group-hover:text-neon-blue transition-all duration-300 group-disabled:cursor-not-allowed">
          {loading ? (
            <>
              ...
              <span>{isVerifiableMode ? 'Creating transaction...' : 'Waiting for next block...'}</span>
            </>
          ) : (
            <>
              ...
              <span>{isVerifiableMode ? 'Generate Verifiable Number' : 'Roll Random Number'}</span>
            </>
          )}
        </div>
      </button>
